  ...
  useEffect(() => {
    if (isCloud && targetFile) {
      fetch(`/admin/api/r2?file=${targetFile}`)
        ...
        .then((cloudData) => {
          if (cloudData) {
            setData(sanitizeAndSyncAbout(cloudData)); 
          }
        })
                ...
                return (
                  <div key={idx} className="flex items-center gap-4 bg-gray-50/50 p-3 rounded-2xl border border-gray-100/70">
                    <div className="w-16 h-16 bg-gray-200/60 rounded-xl border border-gray-200 overflow-hidden shrink-0 flex items-center justify-center relative shadow-inner">
                      {imgName ? (
                        <img src={imgSrc} className="w-full h-full object-cover" alt="Preview" onError={(e) => { e.target.style.display = 'none'; }} />
                      ) : (
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <input type="text" className="flex-1 border border-gray-200 rounded-xl text-xs p-2 bg-white font-mono focus:outline-none focus:border-black transition" value={imgName || ""} onChange={(e) => handleImageNameChange(idx, e.target.value)} placeholder="1.jpg" />
          ...
          {[
            ...
            <div key={lang.key} className="bg-white border border-gray-100 rounded-[24px] p-5 md:p-6 shadow-[0_4px_30px_rgba(0,0,0,0.01)] space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input type="text" className="w-full border border-gray-200 rounded-xl text-xs p-2.5 bg-gray-50/50 focus:bg-white focus:outline-none focus:border-black transition" value={data[lang.key]?.title || ""} onChange={(e) => handleChange(lang.key, "title", e.target.value)} placeholder="請輸入大標題..." />
              </div>
              <div className="space-y-1">
                <label className="block text-[10px] text-gray-400 font-bold uppercase tracking-wider">核心介紹詳細內文 (Content)</label>
                <textarea className="w-full border border-gray-200 rounded-xl text-xs p-3 bg-gray-50/50 h-44 resize-y focus:bg-white focus:outline-none focus:border-black transition leading-relaxed" value={data[lang.key]?.content || ""} onChange={(e) => handleChange(lang.key, "content", e.target.value)} placeholder="請輸入關於自我介紹的完整故事..." />
              </div>
            </div>
          ))}
        </div>
